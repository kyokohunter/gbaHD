...
entity imageGen is 
  port(
    ...
    countX : in std_logic_vector( 10 downto 0 );
    countY : in std_logic_vector( 9 downto 0 );
    skipToY : out std_logic_vector( 9 downto 0 );
    skipTo : out std_logic;
    
    prevLinePrevPxlRgbIn : in std_logic_vector ( 23 downto 0 );
    prevLineCurPxlRgbIn : in std_logic_vector ( 23 downto 0 );
    prevLineNextPxlRgbIn : in std_logic_vector( 23 downto 0 );
    
    curLinePrevPxlRgbIn : in std_logic_vector( 23 downto 0 );
    curLineCurPxlRgbIn : in std_logic_vector( 23 downto 0 );
    curLineNextPxlRgbIn : in std_logic_vector( 23 downto 0 );
    
    nextLinePrevPxlRgbIn : in std_logic_vector( 23 downto 0 );
    nextLineCurPxlRgbIn : in std_logic_vector( 23 downto 0 );
    nextLineNextPxlRgbIn : in std_logic_vector( 23 downto 0 );
    ...
    rgbDat : out std_logic_vector ( 23 downto 0 )
  );
end imageGen;

architecture rtl of imageGen is
constant xMin : integer := 0;
constant yMin : integer := 0; -- -25
...
signal rgbPxl, gridRgb : std_logic_vector( 23 downto 0 );

-- Is a grid pixel?
signal gridAct : std_logic;

constant frameWidth : integer := 1650;
constant frameHeight : integer := 750;
constant videoScreenWidth : integer := 1280;
constant videoScreenHeight : integer := 720;

signal countXDel : integer range xMin to videoScreenWidth;
signal countYDel : integer range yMin to videoScreenHeight;

constant gbaScreenWidth : integer := 240;
constant gbaScreenHeight : integer := 160;
constant gbaVideoScale : integer := 4;
constant gbaVideoWidth : integer := 240 * gbaVideoScale;
constant gbaVideoHeight : integer := 160 * gbaVideoScale; 
constant gbaVideoXStart : integer := (videoScreenWidth - gbaVideoWidth) / 2; --160
constant gbaVideoYStart : integer := (videoScreenHeight - gbaVideoHeight) / 2; --40

-- Signals to be encoded
signal borderRgb : std_logic_vector( 23 downto 0 );
signal rgbDat4b : std_logic_vector( 23 downto 0 );

-- Smoothing
signal rgbSmooth : std_logic_vector( 23 downto 0 );
signal xSelSmooth, ySelSmooth : std_logic_vector( 1 downto 0 );

-- Output signals.
signal rgbTMDSEnc, rgbTERCEnc, 
       rgbNoEnc, rgbNoEnc_del : std_logic_vector( 23 downto 0 );
...
constant vfrontporch : integer := 3; --3
constant vsyncpxl : integer := 5; --5
constant vbackporch : integer := 20; --20
constant hfrontporch : integer := 64; --64
constant hsyncpxl : integer := 36; --36
constant ystart : integer := -( vsyncpxl + vbackporch ); -- -( vsyncpxl + vbackporch ); 
...
--constant dataPreambleXPosStart : integer := 1280 + hfrontporch + hsyncpxl + 10; --1280 + hfrontporch + hsyncpxl + 10
--constant dataXPosStart : integer := dataPreAmbleXPosStart + 8; --dataPreAmbleXPosStart + 8
  ...
  begin
    if ( rising_edge( pxlClk ) ) then
      if ( rst = '1' ) then
        ...
        countXDel <= to_integer(unsigned(countX));
        countYDel <= to_integer(unsigned(countY));
        ...
        if ( to_integer(unsigned(countX)) - 1 = frameWidth ) then
          if ( to_integer(unsigned(countY)) - 1 = frameHeight ) then
            skipTo <= '0';
            ...
            skipTo <= '1';
            skipToY <= std_logic_vector(to_unsigned(gbaVideoYStart - 5, skipToY'length)); --gbaVideoYStart - 5
            newFrameProcessed <= '1';
          else
            skipTo <= '0';
          end if;
        ...
        if ( to_integer(unsigned(countX)) = frameWidth - 1 ) then
          if ( to_integer(unsigned(countY)) = frameHeight or ( newFrameIn = '1' and newFrameProcessed = '0' ) ) then
            lineCnt4 <= 0;
          elsif ( lineCnt4 = 3 ) then
            lineCnt4 <= 0;
          elsif ( to_integer(unsigned(countY)) >= gbaVideoYStart ) then
            lineCnt4 <= lineCnt4 + 1;
          end if;
        end if;
      end if;
...
--  greenNoEnc <= guard1;
  ...
  nextLine <= '0' when ( countXDel /= frameWidth - 8 ) else
              '0' when ( sameLine = '1' ) else
              '0' when ( newFrameIn = '1' and newFrameProcessed = '0' ) else
              '0' when ( to_integer(unsigned(countY)) < gbaVideoYStart ) else
              '1' when ( lineCnt4 = 3 ) else
              '0';
  
  drawGBA <= '1' when ( to_integer(unsigned(countX)) >= gbaVideoXStart and to_integer(unsigned(countX)) < ( gbaVideoXStart + gbaVideoWidth ) and 
                      ( to_integer(unsigned(countY)) >= gbaVideoYStart and to_integer(unsigned(countY)) < ( gbaVideoYStart + gbaVideoHeight ) ) ) else '0';
  
  draw <= '1' when ( countXDel < videoScreenWidth ) and ( countYDel < videoScreenHeight ) else '0';
  
  rgbDat <= rgbPxl when ( drawGBA ='1' and pxlGrid = '0' and smooth2x = '0' and smooth4x = '0' ) else 
            rgbSmooth when ( drawGBA ='1' and ( smooth2x = '1' or smooth4x = '1' ) ) else  
            gridRgb when ( drawGBA ='1' and pxlGrid = '1' ) else
            borderRgb;
...
--    );
  ...
  generic map(
    xMin => xMin,
    xMax => videoScreenWidth,
    yMin => yMin,
    yMax => videoScreenHeight
  )
  port map(
    x => to_integer(unsigned(countX)),
    y => to_integer(unsigned(countY)),
    rgb => borderRgb
  );
  ...
  port map(
    rgbTL => prevLinePrevPxlRgbIn,
    rgbTM => prevLineCurPxlRgbIn,
    rgbTR => prevLineNextPxlRgbIn,
    
    rgbCL => curLinePrevPxlRgbIn,
    rgbCM => curLineCurPxlRgbIn,
    rgbCR => curLineNextPxlRgbIn,
    
    rgbBL => NextLinePrevPxlRgbIn,
    rgbBM => NextLineCurPxlRgbIn,
    rgbBR => NextLineNextPxlRgbIn,
